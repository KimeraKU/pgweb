...
const translations: Record<Language, Translations> = {
  zh: {
    ...
    ugcVideoGenerateCountHint: '最多 8 张候选图',
    ugcVideoAspectRatio: '比例',
    ugcVideoAspectRatioHint: '用于控制商品图输出比例',
    ugcVideoCreativePrompt: '创意提示词',
    ugcVideoCreativePromptPlaceholder: '推荐填写：\n- App 名称\n- 核心卖点（1-3条）\n- 目标用户\n- 使用场景\n- 视频风格（口播/旁白）\n- 结尾引导语（CTA）\n\n示例（App 展示场景）：\nPhotoGrid，一款 AI 修图 App；卖点：一键去背、智能抠图、模板秒出图；目标用户：跨境电商卖家；场景：地铁通勤中快速处理商品图并发布社媒；风格：快节奏产品演示 + 旁白；CTA：立即下载 PhotoGrid，3 分钟完成今日上新素材。',
  ...
  en: {
    ...
    ugcVideoGenerateCountHint: 'Up to 8 candidate images',
    ...
    ugcVideoCreativePromptPlaceholder: 'Recommended inputs:\n- App name\n- Core selling points (1-3)\n- Target users\n- Use scenarios\n- Video style (talking head/voice-over)\n- Ending CTA\n\nExample (App showcase scene):\nPhotoGrid, an AI photo editing app; selling points: one-click background removal, smart cutout, instant templates; target users: cross-border e-commerce sellers; scene: quickly preparing product visuals during a subway commute and posting to social media; style: fast-paced product demo with voice-over; CTA: Download PhotoGrid now and finish today\'s launch assets in 3 minutes.',
    ...
    ugcVideoTaskListImageHint: 'Each candidate image is a separate task',
    ugcVideoTaskListVideoHint: 'Product video tasks are kept here',
    ugcVideoTaskCountUnit: 'ITS',
    ugcVideoImageTab: 'Scene Image',
