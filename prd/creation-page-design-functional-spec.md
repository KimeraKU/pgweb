...
- Price 展开状态使用约 104px 高的简洁紫色渐变卡片，只展示会员层级 `VIP Pro Ultra`、优惠信息 `Save 50%` 和文本式 `View plans` 行动入口；不使用装饰光斑、补充利益点或独立胶囊按钮。
...
侧边栏底部仅保留 Blog、Price 促销卡片和 Language 资源入口，顺序固定不变。
